...
TGDATA_DIR_COAST = './inputData/8721604_NAVD'
TGDATA_DIR_INLAND = './inputData/02248380_NAVD'
...
for scenario in SCENARIOS:
    # For coastal gauge
    for threshold in np.arange(0, 3.65, 0.010):  # Thresholds from 0 to 3 meters
        # Load the json file
        ...
        rounded_threshold = math.floor((threshold - TRIDENT_MHHW)*100)/100  # Adjust threshold to MHHW from NAVD
...
for scenario in SCENARIOS:
    ...
    for threshold in np.arange(0, 3.65, 0.010):  # Thresholds from 0 to 3 meters
        ...
        with open(os.path.join(TGDATA_DIR_INLAND, f'{scenario}/{int(threshold * 100):03d}.json')) as f:
            data = json.load(f)
        # Write the data to a new json file in the MHHW directory
        rounded_threshold = (threshold - HAULOVER_MHHW).round(2)  # Adjust threshold to MHHW from NAVD
        # Format the file name to avoid rounding issues
        formatted_threshold = f"{rounded_threshold:.2f}".replace('.', '').zfill(3)
